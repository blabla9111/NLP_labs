...
5. Slow Growth Penalty -- Штрафует модели с неестественно медленным ростом инфекций
6. Rapid Growth Penalty -- Штрафует модели с нереалистично быстрым распространением
...
5. **RetryChecker (Диспетчер)**
    ...
    Решает, повторить генерацию или завершить процесс. Здесь  происходит передача управления следующему агенту. Нет необходимость в данном агенте делать запрос к LLM, так как решение о том какому агенту дальше передать запрос можно сделать по выводу агентов.
...
4. **WriterAgent (Запись результатов)**
    ...
    Выход: Файл в формате json с результатами 
...
2. Супервизор-рабочий (Supervisor-Worker)
    ...
    - handoff реализован через объект Command (функция need_retry_generator в [main.py](./main.py)), который может и обновить state, и указать какой ноде дальше передать управление.
...
- get_new_comment_from_expert - запрос на получение исправлений (просим пользователя переформулировать комментарий) 

- get_class_subclass_names - классификация комментария на класс и подкласс (собственная модель на основе BERT)


## Управление памятью
- Хранится: История сообщений, состояние сессии, счетчик кол-ва передач управления между агентами PINNLossWeightsAgent и WeightCheckerAgent. Все это хранится в state.

- Очистка: CleanerNode удаляет старые сообщения для экономии места в контексте (функция clear_some_messages в [main.py](./main.py))

- Также используется InMemorySaver для сохранения кратко-срочной памяти между вызовами.
