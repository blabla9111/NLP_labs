...
В данной реализации функция потерь PINN состоит из следующих частей:
...
- check_comment_validity - проверка комментария. Внутри него реализован RetrieverAgent, который отправляет запрос в векторную БД и получает список из комментариев эксперта, которые могут быть похожими на текущий. Затем эти комментарии добавляются в пропмт, чтобы LLM на основе оценила корректность нового комментария, а в случае если он не подходит по критериям корректности, LLM сможет дать рекомендации пользователю по улучшению на базе комментариев из БД. 
