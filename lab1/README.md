...
SciResearch Agent построен на основе LangGraph и реализует многокомпонентный pipeline для комплексных научных исследований:

- **ReAct Agent Node**: Выполняет рассуждения и использует исследовательские инструменты
...
- **Функции Retry**: Для того чтобы сделать систему более надежной, были реализованы функции retry.
    ...
    *  Для большей наглядности и демострации работы функций retry был добавлен код по рандомному выбрациванию ошибок (с низкой вероятностью их появления <0.4)
...
3. Для того чтобы из запроса пользователя выделить четкую тему, ReAct агент при необходимости может вызвать инструмент get_topic, который отправит запрос LLM для выделения главной темы. Ниже пример вывода в консоль информации в процессе работы программы:

![Вывод в консоль о запусках инструментов](terminal_output.png)
6. Как только агент получит всю необходимую информацию для поиска он параллельно отправит запросы по API в arXiv и Crossref. 
7. После получения данных из arXiv и Crossref, запускаются GithubNode для получения ссылкок на репозитории и SummaryNode для извлечения и обобщения информации.
8. Как две эти ноды отработают, запускается WriterNode, который красиво запишет полученную информацию в файл ![research_report.txt](report_result.png)
...
Агент будет обрабатывать исследовательские запросы и генерировать комплексные отчеты ([Пример отчета по поиску](research_report.txt)), включающие:
- Сводки научных статей из arXiv и Crossref
- Релевантные репозитории GitHub
- Структурированные исследовательские находки
