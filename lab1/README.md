...
3. Для того чтобы из запроса пользователя выделить четкую тему, ReAct агент при необходимости может вызвать инструмент get_topic, который отправит запрос LLM для выделения главной темы.
4. Как только агент получит всю необходимую информацию для поиска он параллельно отправит запросы по API в arXiv и Crossref. ![Вывод в консоль о запусках инструментов](terminal_output.png)
